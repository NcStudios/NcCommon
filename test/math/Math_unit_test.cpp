...
TEST(Math_unit_tests, Min_FirstSmaller_ReturnsFirst)
{
    auto smaller = 0u;
    auto larger = 3u;
    EXPECT_EQ(nc::Min(smaller, larger), smaller);
}
...
TEST(Math_unit_tests, Min_SecondSmaller_ReturnsSecond)
{
    auto smaller = 0.0f;
    auto larger = 3.0f;
    EXPECT_EQ(nc::Min(larger, smaller), smaller);
}
...
TEST(Math_unit_tests, Max_FirstLarger_ReturnsFirst)
{
    auto smaller = 0u;
    auto larger = 3u;
    EXPECT_EQ(nc::Max(larger, smaller), larger);
}
...
TEST(Math_unit_tests, Max_SecondLarger_ReturnsSecond)
{
    auto smaller = 0.0f;
    auto larger = 3.0f;
    EXPECT_EQ(nc::Max(smaller, larger), larger);
}
...
TEST(Math_unit_tests, Clamp_ValueLessThanMin_ReturnsMin)
{
    ...
    auto min = 2.0f;
    auto max = 3.0f;
    auto actual = nc::Clamp(value, min, max);
...
TEST(Math_unit_tests, Lerp_FactorZero_ReturnsFrom)
{
    ...
    auto to = 2.0f;
    auto factor = 0.0f;
    auto actual = nc::Lerp(from, to, factor);
...
TEST(Math_unit_tests, WrapAngle_InputLessThanTwoPi_ReturnsInput)
{
    ...
    auto actual = nc::WrapAngle(angle);
...
{
    auto degrees = 180.0;
    auto actual = nc::DegreesToRadians(degrees);
...
TEST(Math_unit_tests, RadiansToDegrees_ConvertsUnits) 
{
    ...
    auto actual = nc::RadiansToDegrees(radians);
